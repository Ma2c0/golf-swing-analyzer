...
(async function () {
  ...
  async function handleStart(btn) {
    const allStartBtns = [btnStart, btnHeroStart, btnCtaStart].filter(Boolean);
    allStartBtns.forEach(b => b.disabled = true);
    ...
    try {
      ...
      PoseModule.setOnPose((landmarks) => {
        ...
        if (allVisible) {
          bodyGuide.querySelector('.guide-outline').style.borderColor = 'rgba(63, 95, 69, 0.8)';
          if (!isRecording) {
            bodyGuide.querySelector('.guide-text').textContent = '✅ Full body detected — show 👍 to start recording';
            bodyGuide.querySelector('.guide-text').style.color = 'rgba(63, 95, 69, 0.8)';
            gestureHint.classList.remove('hidden');
            gestureHint.classList.add('gesture-ready');
          }
        } else {
          bodyGuide.querySelector('.guide-outline').style.borderColor = 'rgba(213, 111, 85, 0.7)';
          bodyGuide.querySelector('.guide-text').textContent = '⚠️ Step back — full body must be visible';
          bodyGuide.querySelector('.guide-text').style.color = 'rgba(213, 111, 85, 0.8)';
          if (!isRecording) {
      ...
      alert(err.message);
      allStartBtns.forEach(b => b.disabled = false);
    }
  }

  btnStart.addEventListener('click', () => handleStart(btnStart));
  if (btnHeroStart) btnHeroStart.addEventListener('click', () => handleStart(btnHeroStart));
  if (btnCtaStart) btnCtaStart.addEventListener('click', () => handleStart(btnCtaStart));
  ...
  function goBackToLanding() {
    ...
    [btnStart, btnHeroStart, btnCtaStart].filter(Boolean).forEach(b => b.disabled = false);
  }

  btnBack.addEventListener('click', goBackToLanding);
  if (btnResultsBack) btnResultsBack.addEventListener('click', goBackToLanding);
